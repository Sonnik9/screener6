...
from config import CFG_PATH, load_config
...
async def maybe_run_benchmark_calibration(
    ...
    for idx, spec in enumerate(benchmark_specs, start=1):
        ...
        cache_name = (
            f"candles_{spec['symbol'].upper()}_"
            f"{spec['start_dt'].strftime('%Y%m%dT%H%M%S')}_"
            f"{spec['end_dt'].strftime('%Y%m%dT%H%M%S')}_"
            f"{spec['step_min']}m.json"
        )
        payload = await run_reverse(
            ...
            candles_cache_path=report_dir / cache_name,
        )
    ...
    _ = load_config(runtime_cfg_path)
