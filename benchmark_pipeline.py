...
def _average_tree(nodes: List[Any], template: Any) -> Any:
    ...
    if isinstance(template, dict):
        ...
        for key, value in template.items():
            out[key] = _average_tree(
                [node.get(key) if isinstance(node, dict) else None for node in valid_nodes],
                value,
            )
...
def _build_filter_from_benchmarks(
    filters: Sequence[Dict[str, Any]],
    base_filter_template: Dict[str, Any],
) -> Dict[str, Any]:
...
async def maybe_run_benchmark_calibration(
    ...
    benchmark_specs = [
        _normalize_benchmark_item(item or {}, default_step_min, default_slot)
        for item in benchmarks_raw
    ]
    ...
    logger.info(f"reverse runtime cfg saved: {runtime_cfg_path}")
    ...
    return runtime_cfg_path, report_payload
